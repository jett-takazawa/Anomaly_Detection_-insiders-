...
5. `insider_likelihood` ∈ [0,100] — overall judgment that the wallet is pursuing a narrow, potentially insider‑advantaged thesis (higher is more insider‑like).
...
- **Overall `insider_likelihood` (0..100):**  
...
- domain_counts: {{"earnings":e,"product":p,"legal":l,"sports":s,"weather":w,"other":o}}
- issuer_counts: {{"TSLA":3,"GOOGL":2,"GENERIC":1}}
- issuer_run_max: {issuer_run_max}
- earnings_titles: {earnings_count} across {earnings_issuers} issuers
- large_positions: {json_large_positions_or_empty_list}
...
async def call_llm(system_prompt: str, user_prompt: str) -> dict:
    ...
    Returns:
        Dict with keys: num_markets, conflicting_earnings, cross_domain_breadth,
                       single_issuer_focus, variant_chain
    """
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        for attempt in range(2):  # Retry once if needed
            try:
                ...
                # Try to extract JSON object from response (may have markdown or text around it)
                # Look for {...} pattern
                ...
                required_keys = ["num_markets", "conflicting_earnings", "cross_domain_breadth",
                                "single_issuer_focus", "variant_chain"]

                if all(key in scores for key in required_keys):
                    # Ensure proper types
                    return {
                        "num_markets": int(scores["num_markets"]),
                        "conflicting_earnings": int(scores["conflicting_earnings"]),
                        "cross_domain_breadth": int(scores["cross_domain_breadth"]),
                        "single_issuer_focus": int(scores["single_issuer_focus"]),
                        "variant_chain": int(scores["variant_chain"]),
                    }
                else:
                    raise ValueError(f"Missing required keys in response: {content}")
                ...
                if attempt == 1:  # Last attempt
                    ...
                    return {
                        "num_markets": 0,
                        "conflicting_earnings": 0,
                        "cross_domain_breadth": 0,
                        "single_issuer_focus": 0,
                        "variant_chain": 0,
                    }
...
if __name__ == "__main__":
    ...
    input_csv = script_dir / "outputs" / "market_users_enriched.csv"
