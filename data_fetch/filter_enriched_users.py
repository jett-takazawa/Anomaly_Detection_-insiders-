"""
Filter users_enriched.csv to only include users from earnings markets.

Reads rated_markets.csv to get valid earnings market IDs, then filters
users_enriched.csv to only keep users who participated in those markets.
"""
...
def main():
    ...
    users_input_path = Path("outputs/users_enriched.csv")
    users_output_path = Path("outputs/users_enriched_earnings_only.csv")
    ...
    print(f"Found {len(valid_market_ids)} valid earnings markets")
    ...
    print(f"\nYou now have {len(filtered_users)} users who participated in earnings markets!")
